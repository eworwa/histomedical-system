 ...
 */
function at_histomedical_form_system_theme_settings_alter(&$form, &$form_state) {
  if (empty(libraries_get_path('font_awesome'))) {
    $library = libraries_detect('font_awesome');
    drupal_set_message(t('You need to download the !font_awesome, extract the archive and place the colorbox directory in the %path directory on your server.', array('!font_awesome' => l(t('Font Awesome'), $library['download url']), '%path' => 'sites/all/libraries/font_awesome')),'error');
  }
  ...
  if (($enable_extensions && $form_state['values']['enable_extensions'] == 1) || (!$enable_extensions && $form['at-settings']['extend']['enable_extensions']['#default_value'] == 1)) {
    ...
    // AT Histomedical does not support box shadows, they dont work well with the overall design.
